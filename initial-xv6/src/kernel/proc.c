...
// Include Round Robin scheduling logic
#else
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        if (min == 0 || p->creation_time < min->creation_time)
          min = p;
      }
      release(&p->lock);
    }
    ...
    for (p = proc; p < &proc[NPROC]; p++)
      {
        ...
        if (p->state == RUNNABLE && p->priority == 0)
        {
          ...
          // Process is done running for now.
          // It should have changed its p->state before coming back.
         // printf("%d %d %d %d\n",p->que0time,p->que1time,p->que2time,p->que3time);
          c->proc = 0;
        }
...
//           // p->queticks=15;
//         }
//         else if (p->state == RUNNABLE && p->priority >= priority && p->priority != 0) // checking for starvation
//         {
//           if (p->wait_time > 3)
//           {
//             p->priority--;
